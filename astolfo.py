...
import psutil, time, os, subprocess as sp, threading, webbrowser, configparser, requests, winsound, shutil, winclip32, json
...
import os
from dotenv import load_init, load_dotenv
load_dotenv()
...
class Config:
    ...
    def _write(self):
        with open(self.location, 'w') as i: self.config.write(i)

    def get(self, section, key, value=False):
        self._update()
        if value: return self.config[section].get(key)
...
class Window(ctk.CTk):
    ...
    def text_field(self):
        ...
        self.textbox.bind('<Return>',
                          lambda event: links.force_start_last_lesson()
    ...
    def check_buttons_frame(self):
        ...
        ctk.CTkCheckBox(frame, text='Discord RPC', variable=discord_var, command=lambda: config.toggle('STATES', 'discord')) \
            .grid(row=0, column=0, sticky='w', padx=10, pady=5)

        inactive_var = ctk.BooleanVar(value=config.get('STATES', 'inactive'))
        ctk.CTkCheckBox(frame, text='Inactive', variable=inactive_var, command=lambda: config.toggle('STATES', 'inactive')) \
            .grid(row=1, column=0, sticky='w', padx=10, pady=5)

        recognition_var = ctk.BooleanVar(value=config.get('STATES', 'recognition'))
        ctk.CTkCheckBox(frame, text='Speech Recognition', variable=recognition_var, command=lambda: config.toggle('STATES', 'recognition')) \
            .grid(row=0, column=1, sticky='w', padx=10, pady=5)

        links_var = ctk.BooleanVar(value=config.get('STATES', 'links'))
        ctk.CTkCheckBox(frame, text='Links', variable=links_var, command=lambda: config.toggle('STATES', 'links')) \
            .grid(row=1, column=1, sticky='w', padx=10, pady=5)

        obs_var = ctk.BooleanVar(value=config.get('STATES', 'obs'))
        ctk.CTkCheckBox(frame, text='OBS', variable=obs_var, command=lambda: config.toggle('STATES', 'obs')) \
    ...
    def week_select_frame(self):
        frame = ctk.CTkFrame(self)
        ctk.CTkLabel(frame, text=f"Тиждень {config.get('DEFAULT', 'week', True)}", font=self.font).pack(expand=True, padx=10, pady=5)
        return frame
...
class SysTray(SysTrayIcon):
    ...
    def yasno_graphic(self, systray):
        img_path = '//'
        days_of_week = ['Понеділок', 'Вівторок', 'Середа', 'Четвер', 'П’ятниця', 'Субота', 'Неділя']
        now = datetime.now()
        timestamp_img = f'{now.strftime("%d.%m")} {days_of_week[now.weekday()]}.png'

        os.rename(img_path + 'yasno-graphic.png', img_path + timestamp_img)
        os.startfile(img_path + timestamp_img)
        os.rename(img_path + timestamp_img, img_path + 'yasno-graphic.png')
...
class MuteSpotify:
    def __init__(self):
        self.state = True
        self.kb = Controller()
        threading.Thread(target=self.shortcuts).start()
...
class DiscordRPC:
    def __init__(self):
        ...
        # self.inactive = inactive.since
        self.description = 'basic'
        # self.current_lesson = links.current_lesson

        self.rpc = Presence('1195490094065397770', pipe=0)

        threading.Thread(target=self.update_rpc).start()
        threading.Thread(target=self.get_all_proc).start()
        
    def update_rpc(self):
        while True:
            if config.get('STATES', 'discord'):

                if not self.connected: # connecting bot
                    try:
                        self.rpc.connect()
                        self.connected = True
                    except: log_error("[Discord] Не вдалось під'єднатись до бота")

                # basic data
                data = {'buttons': [{"label": "літералі мі🥀", "url": "https://i.imgur.com/r0ff8xf.mp4"}]}
                ...
                def activeness():
                    ...
                    if not inactive.since:
                        # str(round(psutil.virtual_memory()[3] / 1000000000, 1))
                ...
                data = {}
                if self.connected: # disconnecting bot
                    self.rpc.close()
                    self.connected = False
    ...
    def get_all_proc(self):
        while True:
            self.description = 'zoom' if 'zoom.exe' in [proc.name().lower() for proc in psutil.process_iter()] else 'basic'
            time.sleep(30)

class OtherHotkeys:
    def __init__(self): # keyboard -> kb module
    ...
    def save_one_mon_screen(self):
        ...
        try:
            with mss() as sct:
                ...
                img_count = [int(name.split(' ')[-1].split('.')[0][1:-1]) for name in
                             os.listdir('C:/Users\LEGION\Pictures\Screenshots')
...
class Process:
    ...
    def start(self, wait_seconds=60):
        ...
        try:
            ...
            # sp.Popen(f'start "" /D "{self.cwd}" "{self.path}"', shell=True)
            sp.Popen([self.path], cwd=self.cwd, stdout=sp.DEVNULL, stderr=sp.DEVNULL, close_fds=True, creationflags=sp.CREATE_NO_WINDOW)
        except Exception as e:
            log_error(f"Failed to start {self.process_name}: {e}")
            return False

        # Чекаємо, поки процес з'явиться
...
class OBS:
    def __init__(self):
        self.connected = False
        self.client = None
        self.process = Process('obs64.exe', r'D:\obs-studio\bin\64bit\obs64.exe', r'D:\obs-studio\bin\64bit')
        self.lock = threading.Lock()  # Додаємо блокування для синхронізації

        threading.Thread(target=self._ensure_connected, daemon=True).start()

    def _ensure_connected(self):
        while True:
            # Використовуємо блокування, щоб гарантувати атомарність
            with self.lock:
    ...
    def execute(self, request):
        if not config.get('STATES', 'obs'): return

        # Використовуємо блокування перед доступом до self.client
        with self.lock:
            if self.connected:
                try:
                    return request()
...
class Inactive:
    ...
    def get_dusk_time(self):
        try:
            res = requests.get('https://api.sunrisesunset.io/json?lat=50.4501&lng=30.5234')
            ...
            # if lastInputTimeDelta > 1000:
                ...
                if lastInputTimeDelta > 10 * 60000:
                    if not self.is_in_deep_sleep:
                        ...
                        if cur_b[0] and (datetime.now().time() > self.dusk_time or datetime.now().time() < self.dawn_time):
                ...
                if not output.startswith(b'Power Scheme GUID: 04d7a134'):
                    # Відновлюємо яскравість, ТІЛЬКИ ЯКЩО ми були в глибокому сні
                    if self.is_in_deep_sleep:
                        try:
                            if self.brightness_temp is not None:
                                # Пробуємо відновити яскравість. Це може впасти після сну.
                                brightness.set(self.brightness_temp[0], 0)
                                self.is_in_deep_sleep = False
                                self.brightness_temp = None
                                log('[Active] Brightness restored')
                        except Exception as e:
                            # Якщо монітор ще не прокинувся, ми не знімаємо прапор is_in_deep_sleep,
                            # щоб спробувати відновити яскравість у наступній ітерації циклу
...
class Links:
    def __init__(self, obs_client_factory):
        ...
        threading.Thread(target=self.links).start()
    ...
    def start_lesson(self, name, les_type, force_start=False, recording=False):
        ...
        # send_tg_message(f'{self.current_lesson}{" - вибіркове" if self.les_links[name]["external"] else ""}\n{self.les_links[name][les_type]}')

        if recording:
            log('[Links] Starting recording in OBS')
            if not self.obs_client: self.obs_client = self.obs_temp_client()
            ...
            for i, lesson in enumerate(self.schedule[week][day]):
                ...
                if lesson and timenow == self.les_start[i]:
                    ...
                    if self.les_links[name][les_type]:
                        ...
                        # повторюємо спробу кожні 30 сек протягом 5 хвилин якщо немає посилання
                        while True: # хз чи работа правильно, перевірю
...
class TGBot:
    ...
    async def force_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if len(context.args) and (context.args[0] not in ['l','w'] or context.args[1] not in ['0', '1']):
            await update.message.reply_text("Usage: /force_start <device(l/w)> <recording(0/1)>\nDefault: linux with no recording")
            return
    ...
    def send_message(self, text):
        asyncio.run(self.bot.sendMessage(os.getenv("TELEGRAM_CHAT_ID"), text, parse_mode=telegram.constants.ParseMode.HTML))
...
# pyinstaller --noconfirm --onefile --windowed --icon "C:/Users/LEGION/PycharmProjects/game/astlofo4.ico" --name "Астольфік"  "C:/Users/LEGION/PycharmProjects/game/helper.py"